...
**Status: optional. Pairs well with [sandboxed autonomy](sandboxed-autonomy.md) and with [the parking protocol](parking-protocol.md) but doesn't require either.**
...
**Scheduled unattended runs.** A queue of well-specified jobs — each one a task clear enough that a model can execute it without back-and-forth clarification — picked up on a schedule (a nightly cron tick is a natural cadence) and run to completion without supervision. The jobs that work best here are the ones where the *thinking* already happened in an earlier synchronous conversation; the unattended run is pure execution against an already-clear spec, not open-ended exploration. A convenient source of well-specified queue entries, if you've adopted it: [parked briefs](parking-protocol.md) with a metadata field marking them ready for unattended pickup — the brief's "next move" section is already the clear spec this mechanism wants, so the two features compose almost for free.
